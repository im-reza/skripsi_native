<table class="mb-0 table" style="font-size: 12px;">
	...
	<tbody>
		...
		if (isset($_POST['cari_sk'])) { 
			...
			$search=$_POST['cari_sk'];
			$page = (isset($_POST['page']))? $_POST['page'] : 1;
			$limit = 5; 
			$limit_start = ($page - 1) * $limit;
			$no = $limit_start + 1;
			$query = mysqli_query($con,"SELECT * FROM surat_keluar INNER JOIN user on surat_keluar.pembuat=user.name where 
				surat_keluar.pembuat like '%".$search."%' or
				surat_keluar.no_surat like '%".$search."%' or
				surat_keluar.tanggal like '%".$search."%' or
				surat_keluar.perihal like '%".$search."%' or
				surat_keluar.penerima like '%".$search."%'
		...
		<?php
		$query_jumlah =mysqli_query($con,"SELECT count(*) AS jumlah FROM surat_keluar  where 
			surat_keluar.pembuat like '%".$search."%' or
			surat_keluar.no_surat like '%".$search."%' or
			surat_keluar.tanggal like '%".$search."%' or
			surat_keluar.perihal like '%".$search."%' or
